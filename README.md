...
이 브랜치는 조회/등록 시연에 필요한 폐기물관리 유스케이스만 구현합니다.

| UC     | 기능                 | 화면 경로        | API                        |
| ------ | -------------------- | ---------------- | -------------------------- |
| UC-W01 | 폐기물 배출 신청     | `/waste/request` | `POST /api/waste-requests` |
| UC-W05 | 폐기물 처리현황 조회 | `/waste/status`  | `GET /api/waste-requests`  |

이 브랜치에서는 승인, 상태 변경, 상세 조회, 수정, 삭제 흐름은 다루지 않습니다.
